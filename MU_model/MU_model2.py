...
given the experimental discharge times as inputs. The muscle is modelled as a single 3-elements Hill-type model,
...
class Params:
    ...
    def __post_init__(self):
        ...
        # Activation defaults pars
        if self.Ca_max_slow is None: 
            self.Ca_max_slow = 285916
        if self.Ca_max_fast is None:
        ...
        # Calcium transient default pars
...
def compute_fs(P: Params, distimes: np.ndarray) -> float:
    distimes = np.atleast_1d(distimes).ravel()
    if distimes.size >= 2:
        return float(np.mean(1.0 / np.diff(distimes)))
    return float(P.stim_freq)
...
class Mechanics:
    ...
    @staticmethod
    def fv_velocity(act: float, l_M_norm: float, f_CE_over_FL: float, FL: float, MU_type: str, vmax: float) -> float:
        fmax = 1.4
        af = 0.17
    ...
    @staticmethod
    def fv_force(v_norm: float, FL: float, act: float, l_M_norm: float, MU_type: str) -> float:
        fmax = 1.4
        af = 0.17
        kMU = 0.5 if MU_type == "slow" else 1.0
        fv = 0.9 + 0.1 * act
...
class Ephys:
    ...
    def activation_dot(self, y, MU_type: str, s: float, muscle: str) -> float:
        ...
        if MU_type == "slow":
            ...
            k1_eff, k2_eff = self.P.k1, self.P.k2
            Ca_eff = Ca
        else:  # fast
            if muscle == "rat_EDL":
                ...
                Ca, a = y[2] * self.P.Ca_max_fast, y[4]
                k1_eff, k2_eff = 32.5, 72.2
            Ca_eff = Ca * (s if s is not None else 1.0)
...
class Systems:
    ...
    def MT(self, t: float, y: np.ndarray, alpha_track: np.ndarray, distimes: np.ndarray, MU_type: str):
        ...
        dsag = self.eph.sag_dot(y[7], t, fs=compute_fs(self.P, distimes))
    ...
    def M_only(self, t: float, y: np.ndarray, distimes: np.ndarray, MU_type: str):
        ...
        dact = self.eph.activation_dot(y, MU_type, y[5], self.P.Ca_max, self.P.k1, self.P.k2, self.P.muscle)
        dsag = self.eph.sag_dot(y[5], t, fs=compute_fs(self.P, distimes))
...
class MuscleModel:
    def __init__(self, P: Params, S: States, distimes):
        ...
        self.fs = compute_fs(P, self.distimes) if self.distimes.size >= 2 else float(P.stim_freq)
    ...
    def run_MT_simulation(self):
        ...
        y0 = [
            ...
            self.S.l_M_0,                  # l_M 
            self.S.y_0,                    # yielding state
            self.S.s_0,                    # sag state
        ]
    ...
    def run_M_simulation(self):
        ...
        y0 = [self.S.MUAP_0, self.S.MUAP_0, self.S.Ca_0, self.S.Ca_0, self.S.act_0, self.S.s_0]
