"""
Author: Andrea Sgarzi
a.sgarzi@ad.unsw.edu.au
University of New South Wales, GSBE
Created on Tue May 21 20:46:53 2024
___________________________________

Tendon force equation given the strain (John 2013)
"""
import numpy as np

def T_force(eps):
    
    # Caillet 2023
    # k1, k2, k3, k4 = 3, 0.025, 42.8, 0.33
    
    # if eps < 0:
    #     f_T = (1+eps)/(10**3)
    # elif eps >= 0 and eps < k2:
    #     f_T = (np.exp((k1*eps)/k2)-1)/(np.exp(k1)-1)
    # elif eps > k2: 
    #     f_T = k3*(eps-k2)+k4
    
    #John 2013
    eps_0 = 0.033 # strain at max. isometric force in rat soleus (5-6% from Monti et al.2003)
    eps_toe = 0.609*eps_0
    #eps_toe = 0.209*eps_0
    #klin = 1.212/eps_0 #1.712
    klin = 1.712/eps_0 #1.712
    F_toe = 0.33
    k_toe = 3
    
    if eps > eps_toe:
        f_T = 0.001*(1+eps)+(klin*(eps-eps_toe)+F_toe)
    elif eps > 0 and eps <= eps_toe:
        f_T = 0.001*(1+eps)+(F_toe*((np.exp(k_toe*eps/eps_toe)-1)/(np.exp(k_toe)-1)))
    else:
        f_T = 0.001*(1+eps)
    
    # if eps > 0:
    #     f_T = 0.001*(1+eps)+(klin*eps)
    # else:
    #     f_T = 0.001*(1+eps)
    
    return f_T