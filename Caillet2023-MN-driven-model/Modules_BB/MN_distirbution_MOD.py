...
def MN_distirbution_func(Nb_MN, true_MN_pop, MN_pool_list, muscle, THRESHOLDS):
    ...
    for i in range (Nb_MN):  #Therefore, it is possible to have MUs indices not always in ascending order within the full pool (I think becase of force oscillating values)
        Real_MN_pop[i]=(  np.abs( Fth_distrib_func(MN_pool_list, muscle, true_MN_pop)-THRESHOLDS[i][2] )  ).argmin() #Taking the min.abs. errors in terms of matching between exp. Fth (%MVC) and full Fth (%MVC) distribution from the literature
