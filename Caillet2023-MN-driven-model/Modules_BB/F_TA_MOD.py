...
def F_TA_func(Transd_Force, MVC, fs, range_start, range_stop, plateau_time1, plateau_time2):
    ...
    Force_Perc_MVC = norm_Force_AU*MVC # Recorded force in % of %MVC
    ...
    F_TA = F_TA- min(F_TA[0:6000])  #remove offset
    # 6. And finally normalize to validate neural drive
    F_TA_norm = F_TA / np.mean(F_TA[plateau_time1*fs:plateau_time2*fs]) #F_TA normalized to mean plateau F_TA
