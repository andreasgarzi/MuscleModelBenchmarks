...
sp_matrix50 = np.empty((Nr,int(t_end_a/T[4])), dtype=float)
...
disch50 = np.arange(0,t_end_a,T[4]) # create array of discharge times at 50 Hz
for i in range (Nr):
    sp_matrix50[i,:] = disch50
    
disch100 = np.arange(0,0.05,T[5]) # create array of discharge times at 100 Hz
for i in range (Nr):
...
# alpha = np.empty((len(fd), Nr, len(time_dt)+1), dtype=object) # Pennation
# l_T = np.empty((len(fd), Nr, len(time_dt)), dtype=object) # Tendon length
# eps_T = np.empty((len(fd), Nr, len(time_dt)), dtype=object) # Tendon strain
# SE_force = np.empty((len(fd), Nr, len(time_dt)), dtype=object) # Tendon force
# CE_force = np.empty((len(fd), Nr, len(time_dt)), dtype=object) # Contractile el. force
# PE_force = np.empty((len(fd), Nr, len(time_dt)), dtype=object) # Parallel elastic el. force
# FL_force = np.empty((len(fd), Nr, len(time_dt)), dtype=object) # Force-length relationship (force)
# vel = np.empty((len(fd), Nr, len(time_dt)), dtype=object) # velocity from FV relationship
# F_M = np.empty((len(fd), Nr, len(time_dt)), dtype=object) # force from FV relationship
