...
def load_Input_Data_func(test, path_to_data):
    ...
    test_cases= np.array([                       
                          ...
                          ], dtype=object)  # list of lists
    
    [test, muscle, plateau_time1, plateau_time2, end_force, MVC, MN_pop, fs]= test_cases[np.argwhere(test_cases==test)[0][0]]  #np.argwhere()[0][0] gives raw index of the match in test_cases (e.g. S1_30_64L gives 2) to then extrapolate data in that raw
    ...
    Data = np.core.defchararray.add(Data, '.npy') #now you have the complete strings of the .npy results file
    
    # Now load .npy results file in corresponding parameters
    ...
    time_MU = np.arange(0, d, dt) #time vector with dt
